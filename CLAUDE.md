...
### 3. 返信ドラフト作成
- **必ず返信案を作成し、ユーザーに提示して送信確認を取ること**
- 確認なしに勝手に送信しない（内容の種別を問わず）
- 提示方法: 「以下の内容で返信します。よろしいですか？」

### 4. 送信（ユーザー承認後のみ）
- ユーザーから「送信して」「OK」等の明示的な承認を得てから送信
- **承認後は以下のコマンドで送信する（draft作成後にdraftIdを使用）**
  ```bash
  GOG_ACCOUNT=riko.gohda@up-sider.com gog gmail drafts send <draftId> --force
  ```
- ビジネスメールは Reply All
- 個人的内容のみ個別返信
