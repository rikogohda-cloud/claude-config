...
**1a. カレンダー予定（両WS）**
```bash
ZONEINFO="C:/Users/NaokiIshigami/.local/lib/zoneinfo.zip" gog calendar list --from=<対象日> --to=<対象日翌日> --account=naoki.ishigami@up-sider.com
```
```bash
ZONEINFO="C:/Users/NaokiIshigami/.local/lib/zoneinfo.zip" gog calendar list --from=<対象日> --to=<対象日翌日> --account=naoki.ishigami@upsidercap.com
```

**1b. メール（両WS）**
```bash
gog gmail search "after:<対象日> before:<翌日>" --account=naoki.ishigami@up-sider.com --max=30
```
```bash
gog gmail search "after:<対象日> before:<翌日>" --account=naoki.ishigami@upsidercap.com --max=30
```

**1c. Slack（UPSIDER本体WS）**
```
mcp__slack__slack_search_messages({ query: "from:naoki.ishigami after:<対象日>", sort: "timestamp", count: 50 })
mcp__slack__slack_search_messages({ query: "to:naoki.ishigami after:<対象日>", sort: "timestamp", count: 50 })
```
